...
echo ============================================================
echo  VividPlate Deployment
echo  Project: %PROJECT%
echo ============================================================
echo.

REM Step 1: Create secrets
echo [1/5] Creating secrets if needed...

gcloud secrets describe DATABASE_URL --project=%PROJECT% --quiet >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
  echo Creating DATABASE_URL...
  <nul set /p =postgresql://neondb_owner:npg_J5TFArU0wejI@ep-calm-poetry-abx9kptm-pooler.eu-west-2.aws.neon.tech/neondb?sslmode=require^&channel_binding=require|gcloud secrets create DATABASE_URL --data-file=- --project=%PROJECT% --quiet
) else (
...
if %ERRORLEVEL% NEQ 0 (
  ...
  <nul set /p =Nx1QZQyys8vi6um2y30dblzyfowic8N45rs+djwkVeSJyQlnMG5rVueSMc7YESxjmd9ChiYdAYN85TlGVjjKKQ==|gcloud secrets create SESSION_SECRET --data-file=- --project=%PROJECT% --quiet
) else (
...
if %ERRORLEVEL% NEQ 0 (
  ...
  <nul set /p =f594b96f30b6804162035a0dcfa13f3ee21fe7be|gcloud secrets create LEMONSQUEEZY_WEBHOOK_SECRET --data-file=- --project=%PROJECT% --quiet
) else (
  echo LEMONSQUEEZY_WEBHOOK_SECRET exists
)

REM Step 2: Grant IAM access
echo.
echo [2/5] Granting IAM access...
...
for %%S in (DATABASE_URL SESSION_SECRET LEMONSQUEEZY_API_KEY LEMONSQUEEZY_WEBHOOK_SECRET) do (
  ...
  echo IAM bound for %%S
)

REM Step 3: Deploy to Cloud Run
echo.
echo [3/5] Deploying to Cloud Run via source...
echo (This builds and deploys - may take 10-15 minutes)
echo.
